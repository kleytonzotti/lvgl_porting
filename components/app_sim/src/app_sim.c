...
static void sim_tick(void)
{
    ...
    // Rampa de transição suave com zona morta anti-oscilação.
    // Evita overshoot ao aplicar desaceleração gradual e "chegada suave"
    // quando está próximo do target.

    float rpm_delta = def.rpm_target - s_rpm_f;

    // Taxa de mudança: aceleração mais forte que desaceleração (física real)
    float rpm_rate = (rpm_delta > 0) ? 32.0f : 18.0f;

    // Zona morta: se está muito próximo do target, vai suavemente
    if (fabsf(rpm_delta) < 80.0f) {
        // Perto do target: reduz taxa para evitar overshoot e oscilar
        rpm_rate *= 0.4f;
    }

    // Aplica a mudança
    if (rpm_delta > rpm_rate) {
        s_rpm_f += rpm_rate;
    } else if (rpm_delta < -rpm_rate) {
        s_rpm_f -= rpm_rate;
    } else {
        // Chegou no target: para por aqui
        s_rpm_f = def.rpm_target;
    }

    // Throttle: responde mais rápido (é comando direto do pedal)
    float thr_delta = def.throttle_target - s_throttle_f;
    float thr_rate = (fabsf(thr_delta) < 15.0f) ? 2.0f : 4.0f;   // mais lento perto do target
    ...
    // Velocidade: segue RPM com inércia (não oscila)
    ...
    float speed_rate = 0.8f;   // mais lento que RPM, suave
